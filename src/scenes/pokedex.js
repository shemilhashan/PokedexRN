...
function PokedexScreen({ navigation }) {
    ...
    const renderItem = ({ item, index }) => (
        <TouchableOpacity style={[styles.tile, { backgroundColor: getCardColor(item.types[0].name), marginRight: index % 2 == 0 ? '1%' : '4%', marginLeft: index % 2 == 0 ? '4%' : '1%'}]} onPress={() => navigation.navigate('Details', { item: item, allData: data, id: index + 1 })}>
            <View style={{flex:1,flexDirection:'row'}}>
                <Text style={styles.name}>{item.name}</Text>
                <View style={{width:40,flexDirection:'column',position:'absolute',top:15,right:5,alignContent:'center',alignItems:'center',justifyContent:'center'}}><Text style={{flex:1,color:'grey',fontWeight:'bold'}}>#{index<9?`00${index+1}`:index<99?`0${index+1}`:`${index+1}`}</Text></View>
            </View>
